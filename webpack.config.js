...
function getVueCheetahGridPath () {
  const vcgPath = path.resolve(__dirname, '../beta-libs/vueCheetahGrid.js')
  try {
    fs.statSync(vcgPath)
    return vcgPath
  } catch (err) {
...
function getCheetahGridPath () {
  const cgPath = path.resolve(__dirname, './beta-libs/cheetahGrid.es5.js')
  try {
    fs.statSync(cgPath)
    return cgPath
  } catch (err) {
  ...
  // context: '/',
    ...
    resolve: {
      ...
      alias: {
        ...
        'cheetah-grid': getCheetahGridPath(),
